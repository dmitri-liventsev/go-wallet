...
// New instantiates HTTP handlers for all the transaction service endpoints
...
func (s *Server) Service() string { return "transaction" }
...
func (s *Server) MethodNames() []string { return transaction.MethodNames[:] }
...
func MountCreateHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/transaction", f)
}

// NewCreateHandler creates a HTTP handler which loads the HTTP request and
// calls the "transaction" service "create" endpoint.
